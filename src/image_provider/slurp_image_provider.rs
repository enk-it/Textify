...
impl SlurpImageProvider {
    fn get_region_from_slurp(&self) -> Result<(i32, i32, u32, u32), Box<dyn std::error::Error>> {
        let output = Command::new("slurp")
            .output()
            .expect("Couldnt start slurp.");

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(format!("Slurp ended with exception: {}", stderr).into());
        }

        let output_str = String::from_utf8(output.stdout)?;
        let parts: Vec<&str> = output_str.trim().split_whitespace().collect();
        if parts.len() != 2 {
            return Err("Wrong slurp format".into());
        }
...
impl super::ImageProvider for SlurpImageProvider {
    fn get_image (&self) -> Result<ImageData, io::Error> {
        let (x, y, width, height) = self.get_region_from_slurp().expect("Erorr");
